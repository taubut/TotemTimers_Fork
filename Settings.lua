...
function TotemTimers.ProcessSetting(setting)
    -- Skip processing if timers aren't initialized yet (happens when GUI opens before addon fully loads)
    if not XiTimers or not XiTimers.timers or not XiTimers.timers[1] then
        return
    end
...
SettingsFunctions = {
    ...
    TwistingTimer = function(value, Timers)
        ...
        for i = 1, 4 do
            if XiTimers.timers[i] and XiTimers.timers[i].nr == AIR_TOTEM_SLOT then
    ...
    Tooltips = function(value, Timers)
        ...
        if TTActionBars and TTActionBars.numbars then
            for i = 1, TTActionBars.numbars do
                if TTActionBars.bars[i] then
    ...
    WeaponBarDirection = function(value, Timers)
        if TotemTimers.WeaponTracker and TotemTimers.WeaponTracker.actionBar then
    ...
    WeaponMenuOnRightclick = function(value, Timers)
        if not XiTimers or not XiTimers.timers or not XiTimers.timers[8] then return end
        if value then
...
if WOW_PROJECT_ID > WOW_PROJECT_CLASSIC then
    SettingsFunctions.HideBlizzTimers = function(value)
        if not TotemFrame then return end -- TotemFrame doesn't exist in TBC Anniversary
        if value then
